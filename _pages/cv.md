...
<ul>{% for p in pubs %}
<li><div class="archive__item"><h4 class="archive__item-title">{% if p.paperurl %}<a href="{{ p.paperurl }}">{{ p.title }}</a>{% else %}{{ p.title }}{% endif %}</h4>{% if p.citation %}<p class="archive__item-excerpt">{{ p.citation }}</p>{% endif %}</div></li>{% endfor %}</ul>
{% endif %}{% endfor %}
